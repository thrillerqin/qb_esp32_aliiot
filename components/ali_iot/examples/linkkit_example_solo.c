...
static const char *TAG = "ALI_IOT_DEBUG";
...
void user_post_property_dht(void)
{
    // dht
    ...
    res = read_dht(&dht_data);
    if (res == DHT_OK)
    {
        ESP_LOGI(TAG, "DHT sensor reading: %.2f / %d", dht_data.temperature, dht_data.humidity);

        HAL_Snprintf(property_payload, sizeof(property_payload), 
                    "{\"Humidity\": %d,\"Temperature\": %.2f}", 
...
void user_post_property_nmea(void)
{
    // gps,nmea0183
    int res;
    //gps_t *pNMEAReading = NULL;
    gps_t nmeaReading;
    char property_payload[128] = {0};

    res = read_nmea_data(&nmeaReading);
    if (res == ESP_OK)
    {
        ESP_LOGI(TAG, "%d/%d/%d %d:%d:%d => \r\n"
                ...
                "\t\t\t\t\t\taltitude   = %.02fm\r\n"
                "\t\t\t\t\t\tspeed      = %fm/s",
                nmeaReading.date.year + YEAR_BASE, nmeaReading.date.month, nmeaReading.date.day,
                nmeaReading.tim.hour + TIME_ZONE, nmeaReading.tim.minute, nmeaReading.tim.second,
                nmeaReading.valid,
                nmeaReading.latitude, nmeaReading.longitude, nmeaReading.altitude, nmeaReading.speed);

        HAL_Snprintf(property_payload, sizeof(property_payload), 
                    "{\"longtitude\": %.6f,\"Latitude\": %.6f,\"Altitude\": %d,\"Speed\": %d}", 
                    nmeaReading.longitude,
                    nmeaReading.latitude, 
                    (int)nmeaReading.altitude, 
                    (int)nmeaReading.speed);
        res = IOT_Linkkit_Report(EXAMPLE_MASTER_DEVID, ITM_MSG_POST_PROPERTY,
                                (unsigned char *)property_payload, strlen(property_payload));
        // ESP_LOGI(TAG, "DHT sensor reading: %.2f / %.2f", dht_data.temperature, dht_data.humidity);

        // HAL_Snprintf(property_payload, sizeof(property_payload), "{\"Humidity\": %d}", dht_data.humidity);
        // res = IOT_Linkkit_Report(EXAMPLE_MASTER_DEVID, ITM_MSG_POST_PROPERTY,
        //                         (unsigned char *)property_payload, strlen(property_payload));

        // HAL_Snprintf(property_payload, sizeof(property_payload), "{\"Temperature\": %.01f}", dht_data.temperature);
        // res = IOT_Linkkit_Report(EXAMPLE_MASTER_DEVID, ITM_MSG_POST_PROPERTY,
        //                         (unsigned char *)property_payload, strlen(property_payload));
    }
    else
...
void user_post_property_switch(void)
{
    static int cnt = 0;
    int res = 0;

    char property_payload[30] = {0};
    HAL_Snprintf(property_payload, sizeof(property_payload), "{\"LightSwitch\": %d}", cnt++%2);
...
int linkkit_solo_main(void)
{
    ...
    IOT_SetLogLevel(/*IOT_LOG_DEBUG*/IOT_LOG_INFO);
    ...
    while (1) {
        ...
        if ((cnt % 2) == 0) {
            //user_post_property();
            user_post_property_switch();
            user_post_property_dht();
            user_post_property_nmea();
        }
